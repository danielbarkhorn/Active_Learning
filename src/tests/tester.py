...
class Tester(object):
    ...
    def graphResults(self):
        ...
        patches = []
        for i in range(len(self.models)):
            patches.append(mpatches.Patch(color=modelColors[i], label=self.models[i][1]))
            plt.plot([size for size in results], means[i], linewidth=3.0, c=modelColors[i],alpha=0.5)
