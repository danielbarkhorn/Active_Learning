...
for sampleSize in range(210, 211, 10):
    print(sampleSize)
    sysSVMF1s = []
    ...
    for _ in range(150):
        ...
        print('fin split',datetime.datetime.now())
        #make random train data and model
        sys_train_PCA = train_pca.systematic_sample(size=250, sort='magnitude')
        print('fin sample',datetime.datetime.now())
        sys_SVM = Model('SVM')
        sys_SVM.fit(sys_train_PCA.get_x(), sys_train_PCA.get_y())
        print('fin fit',datetime.datetime.now())
        sysSVMF1s.append(sys_SVM.test_metric(test_pca.get_x(), test_pca.get_y(), f1=True))
        print('fin append and predict',datetime.datetime.now())
